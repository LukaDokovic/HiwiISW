...
plt.title('Loss Curve with Data Augmentation for Training and Validation')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
# 5. Modellbewertung
test_loss = autoencoder.evaluate(datagen.flow(x_val, x_val, batch_size=64), steps=len(x_val) // 64)
print(f'Test Loss: {test_loss}')

# Rekonstruierte Bilder anzeigen (nur ein Bild)
decoded_img = autoencoder.predict(datagen.flow(x_val, batch_size=1)[0])
...
plt.imshow(decoded_img.reshape(256, 256), cmap='gray')
