...
# 3. Encoder-Decoder-Modell definieren mit angepasster Komplexität und Dropout-Rate
def create_autoencoder(input_shape=(256, 256, 1)):
    model = Sequential()
    
    # Encoder
    model.add(Conv2D(32, kernel_size=(3, 3), activation='relu', padding='same', input_shape=input_shape))  # Filterzahl angepasst
    ...
    model.add(Dropout(0.3))  # Dropout Rate angepasst
...
plt.title('Optimized Loss Curve with Adjusted Dropout, Complexity, and Patience')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
