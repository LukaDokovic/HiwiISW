...
# 3. Encoder-Decoder-Modell definieren mit angepassten Dense Layers
def create_autoencoder(input_shape=(256, 256, 1)):
    ...
    model.add(Flatten())
    model.add(Dense(1024, activation='relu'))  # Dense Layer mit 1024 Neuronen
    model.add(Dense(131072, activation='relu'))  # Dense Layer mit 131072 Neuronen

    # Reshape zurück zu Feature Maps für den Decoder
    model.add(Reshape((32, 32, 128)))  # Formt die Daten in eine Form zurück, die für Conv2D-Schichten geeignet ist
...
plt.title('Optimized Loss Curve with Dense Layers and Learning Rate Adjustment')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
